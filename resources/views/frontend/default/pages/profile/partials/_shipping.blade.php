<h4>{{__('common.make')}} {{__('common.default')}} {{__('defaultTheme.shipping')}} {{__('common.address')}}</h4>
    <table class="table" id="address_table">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{$address->phone}}</td>
                <td>
                    <input type="radio" name="dft_adrs_shipping" value="1" c_list_id="{{$address->id}}"  c_id="{{$address->customer_id}}"@if($address->is_shipping_default==1) checked @endif >
                </td>
            </tr>
            @endforeach
        </tbody>
    ...
    <div class="float-left">
        <button class="btn_1 default_setup_shipping">{{__('common.save')}}</button>
